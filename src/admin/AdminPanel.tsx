...
import { subscribeAdminRealtime, type AdminRealtimeState } from '../lib/adminRealtime';
...
export default function AdminPanel(){
  ...
  const[liveRevision,setLiveRevision]=useState(0);const[liveState,setLiveState]=useState<AdminRealtimeState|null>(null);const[liveStatus,setLiveStatus]=useState<'connecting'|'live'|'offline'>('connecting');const lastLiveRevision=useRef<number|null>(null);
  ...
  useEffect(()=>{
    if(!user){lastLiveRevision.current=null;setLiveState(null);setLiveStatus('connecting');return;}
    setLiveStatus('connecting');
    return subscribeAdminRealtime(state=>{
      setLiveState(state);setLiveStatus('live');
      if(lastLiveRevision.current===null){lastLiveRevision.current=state.revision;return;}
      if(state.revision!==lastLiveRevision.current){lastLiveRevision.current=state.revision;setLiveRevision(value=>value+1);}
    },()=>setLiveStatus('offline'));
  },[user]);

  useEffect(()=>{
    if(!user||liveRevision===0)return;
  ...
  },[user,liveRevision,tab,loadDashboard,loadWithdrawals,loadFlagged,loadEconomy]);
  ...
  const liveLabel=liveStatus==='live'?'AO VIVO':liveStatus==='offline'?'SEM TEMPO REAL':'CONECTANDO';
  const liveColor=liveStatus==='live'?'#34d399':liveStatus==='offline'?'#fb7185':'#facc15';
  return <main className="adm-shell"><aside className="adm-sidebar"><Brand/><nav>{NAV.map(item=>{const Icon=item.icon;return <button key={item.id} className={tab===item.id?'active':''} onClick={()=>item.id==='powerlift'?window.location.assign('/admin/powerlift'):setTab(item.id)}><Icon size={17}/><span>{item.label}</span><ChevronRight size={14}/></button>})}</nav><div className="adm-sidebar-foot"><small>{user.email}</small><button onClick={()=>void signOut(auth)}><LogOut size={16}/>Sair</button></div></aside><section className="adm-content"><div className="adm-topbar"><div><p className="adm-kicker">INVICTUS PERFORMANCE</p><h1>{NAV.find(item=>item.id===tab)?.label}</h1></div><div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',justifyContent:'flex-end'}}><span title={liveState?.lastEventType?`Último evento: ${liveState.lastEventType}`:'Sincronização automática entre app, backend e site'} style={{display:'inline-flex',alignItems:'center',gap:7,padding:'8px 11px',borderRadius:999,border:'1px solid rgba(255,255,255,.09)',background:'rgba(255,255,255,.035)',fontSize:10,fontWeight:900,letterSpacing:'.08em',color:liveColor}}><span style={{width:7,height:7,borderRadius:'50%',background:liveColor,boxShadow:liveStatus==='live'?`0 0 12px ${liveColor}`:'none'}}/>{liveLabel}</span><button className="adm-refresh" disabled={busy} onClick={()=>tab==='dashboard'?void loadDashboard():tab==='withdrawals'?void loadWithdrawals():tab==='flagged'?void loadFlagged():tab==='settings'?void loadEconomy():undefined}><RefreshCw size={16}/>Atualizar</button></div></div>
  ...
  {tab==='revenue'&&<FinanceOverviewPanel key={`revenue-${liveRevision}`} setFeedback={setFeedback}/>} 
  ...
  {tab==='activities'&&<ActivitiesPanel key={`activities-${liveRevision}`} setFeedback={setFeedback} openPending={()=>setTab('flagged')}/>} 
  {tab==='security'&&<SecurityPanel key={`security-${liveRevision}`} setFeedback={setFeedback}/>} 
  {tab==='gyms'&&<GymAuditPanel key={`gyms-${liveRevision}`} setFeedback={setFeedback}/>} 
  {tab==='store-products'&&<StoreProductsPanel key={`products-${liveRevision}`} setFeedback={setFeedback}/>} 
  {tab==='store-drops'&&<StoreDropsPanel key={`drops-${liveRevision}`} setFeedback={setFeedback}/>} 
  {tab==='store-orders'&&<StoreOrdersPanel key={`orders-${liveRevision}`} setFeedback={setFeedback}/>} 
  ...
  {tab==='championships'&&<section className="adm-card"><div className="adm-card-title"><div><small>CAMPEONATOS OFICIAIS</small><h2>Central canônica de campeonatos</h2></div><Trophy/></div><p>Calendário, configuração, publicação, inscrições e operação da edição oficial ficam na central dedicada.</p><a className="adm-primary" href="/admin/championships">Abrir central de campeonatos</a></section>}
