...
export default function ChampionshipsLivePage() {
  ...
  return <main className="pub-page live-champs">
    ...
    <section className="pub-hero live-champs-hero"><div><p>SUPERAÇÃO TEM COMPANHIA.</p><h1>CAMPEONATOS INVICTUS</h1><p className="pub-desc">Escolha sua modalidade, confira datas, valores e inscrições abertas em cada edição.</p></div></section>
    <div className="live-champs-shell">
      ...
      {loading&&!items.length?<div className="pub-loading"><RefreshCw className="spin" size={18}/> Carregando campeonatos...</div>:<div className="live-champs-grid">{items.map(champ => {
        const cardio=champ.type==='run_elite_corrida';
        const href=cardio?'/campeonatos/cardio':'/campeonatos/musculacao';
        return <a className="live-champ-card" href={href} key={`${champ.id}-${champ.editionId}`}><div className="live-champ-img"><span className={champ.registrationOpen?'open':'closed'}>{champ.registrationOpen?'INSCRIÇÕES ABERTAS':'INDISPONÍVEL'}</span>{cardio?<Footprints/>:<Dumbbell/>}</div><div className="live-champ-body"><small>{champ.categoryLabel} · {champ.edition}</small><h3>{champ.title}</h3><p>{champ.subtitle || champ.description}</p><div className="live-champ-meta"><span><CalendarDays/> {period(champ.startAt,champ.endAt)}</span><span><Trophy/> {money(champ.registrationPrice)}</span></div>{!champ.registrationOpen&&<em>{champ.registrationReadinessReason || 'Inscrições ainda não abertas.'}</em>}<b>Ver detalhes <ArrowRight size={14}/></b></div></a>})}</div>}
      <section className="pub-card live-ecosystem"><p className="pub-eyebrow">OUTRAS FORMAS DE COMPETIR</p><h2>Mais formas de entrar no jogo</h2><div><a href="/entre-amigos"><Users/><b>Entre Amigos</b><span>Desafios privados com IGA e prêmio opcional em Invictus Coins.</span></a><a href="/power-lift"><Dumbbell/><b>Power Lift</b><span>Progressão sazonal, marcas auditadas e rankings Elite/Geral.</span></a><a href="/conta"><ShieldCheck/><b>Minha Conta</b><span>Acompanhe suas inscrições e participações.</span></a></div></section>
    </div>
  </main>;
}
